...
public class DeserializationData {
    ...
    public static  List<String> getPairIntTypeValueRecords(String avroFilePath){

        List<String> records = new ArrayList<>();
        try {

            Schema schema = Pair.getPairSchema(Schema.create(Type.INT),TypeValue.getClassSchema());
            ...
            while (fileReader.hasNext()) {
                ...
                String type = valueRecord.get("type").toString();
                String albums = valueRecord.get("albums").toString();

                Pair<Integer, DayValue> pair = new Pair<>(year, new TypeValue(type, albums));
